...
cf_buffer *c_file_interpreter(char *file_name, project *p, mcp *profile, stack *history)
{
            ...
            if (i > 0)
            {
                ...
                case inclusion:
                    if ((textm == double_quote) && cb == '"')
                    {
                        ...
                        {
                            if (stack_lstr_search(p->headers, identifier))
                            {
                                if (!stack_lstr_search(buffer_ret->headers, identifier))
                                {
                                    if (!stack_lstr_search(buffer_ret->history, identifier))
                                    {
                                        printf("\tHEADERS: %s from %s\n", identifier, file_name);

                                        // printf("goto %s\n", identifier);

                                        cf_buffer *buffer_carrier =
                                            c_file_interpreter(identifier, p, profile, buffer_ret->history);
                                        if (!buffer_carrier)
                                        {
                                            break;
                                        }
                                        ...
                                        stack_str_append(buffer_ret->headers, buffer_carrier->headers);
                                        stack_str_append(buffer_ret->external_files, buffer_carrier->external_files);
                                        stack_str_append(buffer_ret->function_declarations,
                                                         buffer_carrier->function_declarations);
                                        stack_str_append(buffer_ret->function_references,
                                                         buffer_carrier->function_references);
                                        stack_str_append(buffer_ret->history, buffer_carrier->history);
    ...
    for (int i = 0; i < stack_function->stack_index; i++)
    {
        for (int j = 0; j < lib_buffer_stack->stack_index; j++)
        {
            ...
            if (!strcmp(lib_buffer->name, "project"))
            {
